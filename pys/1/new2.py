...
def find_matches(base, target, common_fields):
    ...
    # Adding summary with counters
    total_base_rows = len(base)
    total_target_rows = len(target)
    total_matches = len(matched_rows)
    unmatched_base = total_base_rows - len(matches['base_match_key'].unique())
    unmatched_target = total_target_rows - len(matches['target_match_key'].unique())
    ...
    st.write(f"Unmatched rows in base dataset: {unmatched_base}")
    st.write(f"Unmatched rows in target dataset: {unmatched_target}")
...
if uploaded_file1 and uploaded_file2:
    ...
    if len(common_fields) > 0:
        ...
        if st.button("Merge fields"):
            merged = target.merge(matched_rows[[col for col in additional_fields + ['match_key'] if col in matched_rows.columns]], on='match_key', how='left')
