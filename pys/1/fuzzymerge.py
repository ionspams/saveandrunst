...
def fuzzy_match(row, df, keys, thresholds):
    for _, target_row in df.iterrows():
        matches = [fuzz.ratio(row[key], target_row[key]) >= threshold for key, threshold in zip(keys, thresholds)]
        if all(matches):
            return target_row
    return None
...
if base_file and target_file:
    ...
    for col in match_cols:
        thresholds.append(st.slider(f"Fuzzy Matching Threshold for {col}", 0, 100, 80))
    ...
    if match_cols:
        ...
        if st.button("Match and Transpose"):
            ...
            for i, base_row in base_data.iterrows():
                matched_row = fuzzy_match(base_row, target_data, match_cols, thresholds)
