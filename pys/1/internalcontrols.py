...
SERVICE_ACCOUNT_FILE = 'C:\\Users\\ghici\\Documents\\Scanned Documents\\docstreamer\\docstreamerAPI.JSON'
...
# Open specific worksheets with error handling
def get_worksheet(spreadsheet_id, worksheet_name):
    try:
        worksheet = gc.open_by_key(spreadsheet_id).worksheet(worksheet_name)
        return worksheet
    except gspread.exceptions.WorksheetNotFound:
        st.error(f"Worksheet '{worksheet_name}' not found in spreadsheet '{spreadsheet_id}'.")
        return None

employees_sheet = get_worksheet(SPREADSHEET_ID_2, 'Employees')
shifts_sheet = get_worksheet(SPREADSHEET_ID_2, 'Shifts')
guests_sheet = get_worksheet(SPREADSHEET_ID_2, 'Guests')
tender_sheet = get_worksheet(spreadsheet_id_3, 'Tenders')
bidders_sheet = get_worksheet(spreadsheet_id_3, 'Bidders')
review_sheet = get_worksheet(spreadsheet_id_3, 'Review')
...
def update_bid_status(bid_id, new_status):
    bidders_sheet.update_cell(bid_id + 2, 7, new_status)  # Assuming status is in the 7th column
