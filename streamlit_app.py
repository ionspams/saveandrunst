...
def save_code_and_run_with_dependencies(code_input, file_name):
    try:
        ...
        if os.name == 'nt':
            command = f'{activate_script} & streamlit run {script_path} --server.port {free_port}'
        else:
            command = f'bash -c ". {activate_script} && streamlit run {script_path} --server.port {free_port}"'
